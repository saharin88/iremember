...
const getPeopleFilteredByYear = async () => {
    params.value[`filter[${route.params.type}_year]`] = route.params.year
    return getPeople(params.value)
}
provide('getPeople', getPeopleFilteredByYear)

const title = wTrans(`People filtered by year of ${route.params.type} :year`, {year: route.params.year})
    ...
    <Loading :isLoading="isLoading">
        ...
        <div class="justify-center items-center w-full flex">
            <TailwindPagination
                :data="people"
                :limit="5"
                @pagination-change-page="page => { params.page = page; getPeopleFilteredByYear()}"
                class="mt-4"
            />
        </div>
